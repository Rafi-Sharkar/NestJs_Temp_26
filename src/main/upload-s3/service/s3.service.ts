...
import { compressImageIfRequired } from '@/common/utils/file-compression.util';
...
export class S3Service {
  ...
  async uploadFile(file: Express.Multer.File) {
    const processedFile = await compressImageIfRequired(file);
    const fileExt = processedFile.originalname.split('.').pop();
    const folder = this.getFolderByMimeType(processedFile.mimetype);
    ...
    const fileRecord = await this.prisma.client.fileInstance.create({
      data: {
        ...
        path: s3Key,
        url: fileUrl,
        fileType: this.getFileType(processedFile.mimetype),
